package utils
{
	...
	/**
	 * Originally Filter.as
	 * Current class was optimized.
	 *
	...
	public class BBFilter
	{
		...
		static private const PI_div_2:Number = Math.PI / 2;
		static private const DEG_TO_RAD:Number = Math.PI / 180.0;
		static private const RAD_TO_DEG:Number = 180.0 / Math.PI;

		static private var sinTable:Vector.<Number>;
		static private var cosTable:Vector.<Number>;
		 ...
		 *
		 * @param p_source     BitmapData  The input bitmap data to apply the twirling effect.
		 * @param p_region     Rectangle   The region to apply the twirling effect.
		 * @param p_rotation   Number      The max amount to rotate, in radius.
		 *                                Default is Math.PI / 2.
		 *
		 * @return DisplacementMapFilter    Filter to apply twirl effect
		 */
		static public function twirlFilter(p_source:BitmapData, p_region:Rectangle = null, p_rotation:Number = 0):DisplacementMapFilter
		{
			initSinCos();
			...
			var radius:Number = Math.min(width, height) * 0.5;
			var centerX:int = xPos + width * 0.5;
			var centerY:int = yPos + height * 0.5;
			var xC:int;
			var yC:int;
			var yCS:int;
			var dr:Number;
			...
			var sinus:Number;
			var angleDeg:int;
			var cosTab:Vector.<Number> = cosTable;
			var sinTab:Vector.<Number> = sinTable;
			...
			for (var y:int = 0; y < height; y++)
			{
				...
				{
					xC = x - centerX;
					dr = radius - Math.sqrt(xC * xC + yCS);

					if (dr > 0)
					{
						angle = dr / radius * p_rotation;

						angleDeg = angle * RAD_TO_DEG;
						cosin = cosTab[angleDeg];
						sinus = sinTab[angleDeg];

						dx = xC * cosin - yC * sinus - xC;
						dy = xC * sinus + yC * cosin - yC;

						round = dx / width * 0xff;
						round += round < 0 ? -0.5 : 0.5;
						blue = 0x80 + round;

						round = dy / height * 0xff;
						round += round < 0 ? -0.5 : 0.5;
						green = 0x80 + round;

						dbmd.setPixel32(x, y, green << 8 | blue);
					}
				}
			}
		 ...
		 *                                Default is 0.35.
		 */
		static public function pinchFilter(source:BitmapData, region:Rectangle = null, amount:Number = 0.35):DisplacementMapFilter
		{
		 ...
		 */
		static public function bulgeFilter(source:BitmapData, region:Rectangle = null, amount:Number = 0.5):DisplacementMapFilter
		{
		 ...
		 */
		static public function squeezeFilter(source:BitmapData, region:Rectangle = null, amount:Number = 0.5):DisplacementMapFilter
		{
		 ...
		 */
		[Inline]
		static private function cos(p_angle:Number):Number
		{
			return cosTable[int(p_angle * RAD_TO_DEG)];
		}

		/**
		 */
		[Inline]
		static private function sin(p_angle:Number):Number
		{
			return sinTable[int(p_angle * RAD_TO_DEG)];
		}

		/**
		 */
		static private function initSinCos():void
		{
			if (sinTable == null)
			{
				sinTable = new <Number>[];
